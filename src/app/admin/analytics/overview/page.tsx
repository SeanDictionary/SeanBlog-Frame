      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(22rem,0.9fr)]">
        ...
        <div className="min-w-0 space-y-6">
          ...
          <section className="rounded-lg border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-950">
            ...
            <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-xs text-neutral-500"><tr><th className="py-2">文章</th><th className="py-2 text-right">访问量</th><th className="py-2 text-right">访问人数</th></tr></thead><tbody className="divide-y divide-neutral-100 dark:divide-neutral-900">{data.topArticles.map((article) => <tr key={article.slug} className="cursor-pointer transition-colors hover:bg-neutral-50 dark:hover:bg-neutral-900/60"><td className="py-3"><a href={metricHref({ dimension: 'article', slug: article.slug })} className="font-medium">{article.label}</a><span className="mt-0.5 block font-mono text-xs text-neutral-500">{article.slug}</span></td><td className="py-3 text-right">{article.views}</td><td className="py-3 text-right">{article.visitors}</td></tr>)}{data.topArticles.length === 0 && <tr><td colSpan={3} className="py-8 text-center text-neutral-500">暂无文章访问数据。</td></tr>}</tbody></table></div>
          </section>
