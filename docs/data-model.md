...
- UA 解析口径：`parseBrowser` 先判爬虫（`/bot|crawler|spider|slurp|externalhit|externalagent/i`，含 Google/Bing/Baidu/Sogou/字节/GPTBot/ClaudeBot 等，新 -bot 命名爬虫天然免疫），再判 App 内置 WebView（微信/微博/支付宝/钉钉/飞书/QQ）→ 国产浏览器（UC/360/搜狗/猎豹/百度/夸克/遨游）→ 手机厂商浏览器（华为/小米/OPPO/vivo/三星）→ Edge（含 `Edg/EdgA/EdgiOS`）→ Opera（`OPR/`/`Opera/`/`OPiOS/`）→ Chrome（含 iOS `CriOS/`）→ Firefox（含 iOS `FxiOS/`）→ Safari。标签只取品牌/产品名，不带"浏览器"后缀；App webview 与厂商浏览器 UA 多含 `Chrome/`（Chromium 内核），必须先于 `Chrome/` 规则按特征 token 优先识别，否则会被 Chrome 兜底吞掉；iOS 上 Chrome/Firefox/Opera 用 WKWebView，UA 用 `CriOS/`/`FxiOS/`/`OPiOS/` 而非 `Chrome/`/`Firefox/`/`OPR/`，单独识别。`parseOperatingSystem` 先判 iOS（`iPhone|iPad|iPod`，先于 `Mac OS X` 以免 iPhone/iPad 的 `like Mac OS X` 被误判为 macOS）→ Windows/macOS/Android/Linux
