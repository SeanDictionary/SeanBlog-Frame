...
- github-repo / friend-link 卡片指令现同时支持三种写法：容器指令（`:::github-repo{...}\n:::`）、叶子指令（`::github-repo{...}` 两个冒号，推荐单行写法）、行内指令（`:github-repo{...}` 一个冒号）。此前仅支持容器指令。叶子指令和行内指令的多个连续卡片能正确包裹进网格。行内指令嵌在普通文本中间时，卡片会被提取为独立块（HTML 规范限制 `<p>` 不能包含块级元素）。使用文档同步更新语法示例。README 新增「使用指南」板块链接到完整使用文档。
